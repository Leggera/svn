...
def main(corpora, p2v_dir, p2v_file, diag_dir, epoch):
    ...
    if ('IMDB' in corpora):
        dev = 10
        p2v_DocumentVectors0 = np.array([p2v_model.docvecs['SENT_'+str(i)] for i in range(12425, 12575 - dev)] + [p2v_model.docvecs['SENT_'+str(i)] for i in range(24925+dev, 25075)])
        y_1 = [1] * (75 - dev)
        y_0 = [0] * (75 - dev)
