...
alphas=('-alpha 0.01' '-alpha 0.02' '-alpha 0.03' '-alpha 0.04' '-alpha 0.05' '-alpha 0.06'
'-alpha 0.07' '-alpha 0.08' '-alpha 0.09' '-alpha 0.1' '-alpha 0.15' '-alpha 0.2')
  ...
  for m_c in "${min_count[@]}"; do
      for model in "${default_models[@]}"; do
        for alpha in "${alphas[@]}"; do
            ...
            (time (python3 run_small_IMDB_alpha02_5epochs.py -output "$space_fold""$d2v_out" $iter $m_c $alpha $model $d_p >> "$d2v_t")) &>> "$d2v_t" &
        done
	wait
      done
  done
