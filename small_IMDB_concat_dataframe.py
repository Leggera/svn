...
def DocumentVectors(model, model_name):
    ...
    if (model_name == "word2vec_c"):
        ...
        DocumentVectors0 = np.array([model_d2v.docvecs['SENT_'+str(i)] for i in range(12000, 12500-d)]+[model_d2v.docvecs['SENT_'+str(i)] for i in range(12500+d, 13000)]) #first 25000 are labeled train data
        f = open(model + 'test', 'rb')
        p = pickle.load(f)
        DocumentVectors1 = np.concatenate([p[i][0].reshape(1, -1) for i in p])
        #DocumentVectors1 = [model_d2v.docvecs['SENT_'+str(i)] for i in range(62000, 63000)]#second 25000 are labeled test data
