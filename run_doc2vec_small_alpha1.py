...
def run_doc2vec(train_docs, dev_docs, test_docs, dm, size, window, alpha, alpha1, negative, sample, cores, min_count, passes, output, diagnostics = False):

    assert gensim.models.doc2vec.FAST_VERSION > -1, "this will be painfully slow otherwise"

    model = Doc2Vec(dm=dm, size=size, window=window, alpha = alpha1, min_alpha = alpha1, negative=negative, sample=sample, workers=cores, min_count = min_count, iter = 1)
